...
export function MiniNavbar() {
  ...
  return (
    ...
    `}>
      <div className="flex items-center justify-between w-full gap-x-6 sm:gap-x-8">
        ...
        <div className="hidden sm:flex items-center gap-3">
          {status === 'loading' ? null : session ? (
            ...
            <div className="flex items-center gap-6">
            <p className=" text-white/70 font-light">Join us</p>
            <button
      ...
      {/* mobile menu */}
      <div className={`
        sm:hidden flex flex-col items-center w-full overflow-hidden
        ...
        <div className="flex flex-col items-center space-y-4 mt-4 w-full">
          {status === 'loading' ? null : session ? (
            <div>
              <button
                onClick={() => signOut({ callbackUrl: '/' })}
                className="px-4 py-2 text-sm border border-[#333] bg-[rgba(31,31,31,0.62)] text-gray-300 rounded-full w-full hover:border-white/50 hover:text-white transition-colors duration-200"
              >
                Sign out
              </button>
              <nav className="flex flex-col items-center space-y-4 text-base w-full">
                {navLinksData.map((link) => (
                  <a
                    key={link.href}
                    href={link.href}
                    className="text-gray-300 hover:text-white w-full text-center"
                  >
                    {link.label}
                  </a>
                ))}
            ...
            <button
              onClick={() => signIn('google', { callbackUrl: '/home' })}
              className="px-4 py-2 text-sm font-semibold text-black bg-gradient-to-br from-gray-100 to-gray-300 rounded-full w-full hover:from-gray-200 hover:to-gray-400 transition-all duration-200"
            >
              Sign up
            </button>
            </div>
                ...
                {step === "email" ? (
                  ...
                  >
                    <div className="space-y-1">
                      <h1 className="text-[2.5rem] font-bold leading-[1.1] tracking-tight text-white">BlinkPay</h1>
                      <p className="text-[1.8rem] text-white/70 font-light">Click. Blink. Transfer</p>
                    </div>


                    <div className="space-y-4">
                      <button onClick={() => { signIn("google", { callbackUrl: '/home' }) }} className="backdrop-blur-[2px] w-full flex items-center justify-center gap-2 bg-white/5 hover:bg-white/10 text-white border border-white/10 rounded-full py-3 px-4 transition-colors">
                    ...
                    {/* <p className="text-xs text-white/40 pt-10">
                      By signing up, you agree to the, <Link href="#" className="underline text-white/40 hover:text-white/60 transition-colors">Policies</Link>
                    </p> */}
                  </motion.div>
                ) : step === "code" ? (
                  <motion.div
