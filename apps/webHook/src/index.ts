  ...
  const amount = decoded.Xtoken.amount;
        try{
          ...
          await prisma.$transaction(async (tx) => {
            ...
            const onRamping = await tx.onRamping.update({
              where: { token: Xtoken },
              data: {
            ...
            const balance = await tx.balance.upsert({
              ...
              update: { amount: (Number({ increment: amount}))},
              create: {
                user_id: userId,
                currency: "INR", 
              },
            ...
            console.log("OnRamping updated:", onRamping);
            console.log("Balance updated:", balance);
            })
          
        } catch (error) {
            console.error("Error processing prisma webhook payload:", error);
            return res.status(400).json({ status: "error", message: "payload - prisma call webhook" });
        }
    
        res.status(200).json({ status: "success", message: "Webhook received and balance updated" });
    } catch (error) {
        console.error("Error processing webhook route", error);
        res.status(500).json({ status: "error", message: "Internal server  - webhook route" });
    }
});
